...
import { getBackendErrorMessage, runAutonomyExecution } from "../services/autonomyService";
...
async function handleFixNow(actionId: string) {
  ...
  try {
    const data = await runAutonomyExecution({ diagnosedCategory: result.diagnosed_category });
    const latestAttempt = data.attempts?.[data.attempts.length - 1];
    setActionResults(prev => ({
      ...prev,
      [actionId]: {
        success: data.execution?.success === true,
        summary:
          latestAttempt?.notes ||
          data.execution?.summary ||
          data.safety?.rejectionReason ||
          data.verification ||
          "Backend autonomy request completed.",
      },
    }));
  } catch (error) {
    setActionResults(prev => ({
      ...prev,
      [actionId]: { success: false, summary: getBackendErrorMessage(error) },
    }));
  } finally {
  ...
  <div style={styles.card}>
    ...
    <div style={{ marginBottom: 14 }}>
      <AutonomyRemediationPanel diagnosedCategory={result.diagnosed_category} />
    </div>
