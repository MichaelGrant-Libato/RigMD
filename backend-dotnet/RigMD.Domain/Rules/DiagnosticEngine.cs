...
public static class DiagnosticEngine
{
    ...
    private static DiagnosticProofItem _MakeProof(
        string label,
        string value,
        string status,
        string meaning)
    ...
    private static string _ProofStatus(
        List<DiagnosticProofItem> proof,
        string label)
    {
        var item = proof.FirstOrDefault(
            p => string.Equals(
                p.Label,
                label,
    ...
    private static string _ProofValue(
        List<DiagnosticProofItem> proof,
        string label)
    {
        var item = proof.FirstOrDefault(
    ...
    private static bool _HasPressure(
        List<DiagnosticProofItem> proof,
    ...
    private static bool _EvidenceHas(
        List<DiagnosticEvidenceItem> evidence,
        params string[] words)
    {
        var combined = string.Join(
            " ",
            evidence.Select(e => $"{e.Label} {e.Value}"));

        return _Has(combined, words);
    }

    public static DiagnosticResult RunDiagnostic(
        DiagnosticSymptomPayload sym,
        HardwareMetrics metrics)
    {
        var warningText =
            _JoinedText(
                sym.WarningSigns,
                sym.WarningSignsLabel,
                sym.SystemState);

        var recentChangesText =
            _JoinedText(
                sym.RecentChanges,
                sym.RecentChangesLabel,
                sym.SystemState);

        var activityText =
            _JoinedText(
                sym.AffectedActivity,
                sym.AffectedActivityLabel,
                sym.SystemState);

        var proof = new List<DiagnosticProofItem>();

        var cpuStatus =
            metrics.CpuUsagePercent >= 85
                ? "high"
        ...
        var ramStatus =
            metrics.RamUsagePercent >= 85
                ? "high"
                : metrics.RamUsagePercent >= 75
                    ? "elevated"
                    : "normal";
        ...
        if (
            metrics.BrowserMemoryMb >= 2048 ||
            metrics.BrowserProcessCount >= 20)
        {
            browserStatus = "high";
        }
        else if (
            metrics.BrowserHeavy ||
            metrics.BrowserMemoryMb >= 1024 ||
            metrics.BrowserProcessCount >= 10)
        {
            browserStatus = "elevated";
        }
        else if (metrics.BrowserMemoryMb > 0)
        {
            browserStatus = "observed";
        }

        string gameStatus =
            metrics.GameDetected
                ? "observed"
                : "normal";

        string gameValue =
            metrics.GameProcesses.Any()
                ? string.Join(
                    ", ",
                    metrics.GameProcesses.Take(3))
                : "None detected";
        ...
        proof.Add(
            _MakeProof(
                ...
                cpuStatus,
                "High CPU load can cause stuttering, freezing, and slow response."));
        ...
        proof.Add(
            _MakeProof(
                ...
                ramStatus,
                "High RAM usage can cause freezing, slow app switching, and browser lag."));

        proof.Add(
            _MakeProof(
                "Storage usage",
                $"{metrics.DiskUsagePercent}% full",
                diskStatus,
                "Very full storage can slow updates, file access, and system maintenance."));

        proof.Add(
            _MakeProof(
                "Browser workload",
                $"{metrics.BrowserMemoryMb} MB across {metrics.BrowserProcessCount} processes",
                browserStatus,
                "Heavy browser memory usage can explain slowdowns with many tabs or web apps."));
        ...
        proof.Add(
            _MakeProof(
                ...
                gameValue,
                gameStatus,
                "Game or launcher processes can increase GPU, CPU, memory, and heat while they are open."));

        /*
         * CPU frequency reduction under high utilization is treated
         * as a heuristic. It does not directly confirm overheating.
         */
        if (metrics.CpuThermalThrottling)
        {
            proof.Add(
                _MakeProof(
                    ...
                    "High CPU load with reduced reported frequency",
                    "elevated",
                    "The observed combination may indicate frequency throttling, but it does not directly prove overheating."));
        }

        /*
         * The current storage-health flag represents Windows
         * reporting an abnormal physical-drive status. It does not
         * directly prove a SMART predictive failure.
         */
        if (metrics.SmartDriveWarning)
        {
            proof.Add(
                _MakeProof(
                    ...
                    "Windows reported a non-OK drive status",
                    "high",
                    "The physical drive reported an abnormal Windows health status. Back up important data and verify the drive with a dedicated storage-health tool."));
        }

        /*
         * A single high process-memory reading does not establish
        ...
        if (!string.IsNullOrEmpty(metrics.MemoryLeakWarning))
        {
            proof.Add(
                _MakeProof(
                    ...
                    metrics.MemoryLeakWarning,
                    "elevated",
                    "A process is using an unusually large amount of memory. This may indicate a software issue or possible memory leak, but a single reading does not confirm one."));
        }
        ...
        bool criticalWarning =
            _Has(
                ...
                "file not found",
                "display driver") ||
            sym.MentionsBlackScreen ||
            sym.MentionsNoBoot ||
            sym.MentionsBlueScreen ||
            sym.MentionsStorage;

        bool objectiveProblemFound =
            proof.Any(
                p =>
                    p.Status == "elevated" ||
                    p.Status == "high");

        bool strongUserWarning =
            criticalWarning ||
            (
                sym.Severity == "high" &&
                (
                    sym.Frequency == "frequent" ||
                    sym.Frequency == "always"
                )
            );
        ...
        if (browserStatus == "high")
        {
            _Add(
                ...
                "OS performance degradation",
                4,
                "Heavy browser workload detected",
                $"{metrics.BrowserMemoryMb} MB across {metrics.BrowserProcessCount} processes",
                "Live scan");
        }
        else if (browserStatus == "elevated")
        {
            _Add(
                score,
                evidence,
        ...
        else if (metrics.DiskUsagePercent >= 80)
        {
            _Add(
                ...
                "OS performance degradation",
                1,
                "Storage usage elevated",
                $"{metrics.DiskUsagePercent}% full",
                "Live scan");
        }

        if (
            _Text(metrics.StorageType) == "hdd" &&
            (
                sym.AffectedActivity == "startup" ||
                sym.AffectedActivity == "working"
            ))
        {
            _Add(
                score,
                ...
                1,
                "Storage type",
                "HDD can contribute to slow loading",
                "Detected component");
        }

        if (
            metrics.GameDetected &&
            sym.AffectedActivity == "gaming")
        {
            if (
                reportedCategory == "Thermal condition" ||
                sym.MentionsHeat)
            {
                _Add(
                    score,
                    ...
                    "Thermal condition",
                    2,
                    "Game workload observed",
                    gameValue,
                    "Live scan");
            }

            if (
                reportedCategory == "Display driver behavior" ||
                sym.MentionsFlicker)
            {
                _Add(
                    score,
        ...
        if (metrics.CpuThermalThrottling)
        {
            _Add(
                ...
                "Thermal condition",
                6,
                "Possible frequency throttling observed",
                "High CPU utilization with reduced reported frequency",
                "Live scan");
            ...
            _Add(
                ...
                "OS performance degradation",
                3,
                "Performance reduction observed",
                "Reduced reported CPU frequency under high load",
                "Live scan");
        }
        ...
        if (metrics.SmartDriveWarning)
        {
            _Add(
                ...
                "Storage health behavior",
                10,
                "Abnormal physical drive health status",
                "Windows reported a non-OK drive status",
                "Live scan");
        }
        ...
        if (
            ...
            reportedCategory == "OS performance degradation")
        {
            _Add(
                score,
                evidence,
        ...
        if (
            _Has(
                ...
                "driver update") &&
            (
                reportedCategory == "Driver conflict" ||
                reportedCategory == "Display driver behavior"
            ))
        {
            _Add(
        ...
        if (
            ...
            (
                reportedCategory == "OS performance degradation" ||
                reportedCategory == "Boot and startup failure" ||
                reportedCategory == "Driver conflict"
            ))
        {
            _Add(
        ...
        if (
            _Has(
                ...
                "hardware upgrade") &&
            (
                reportedCategory == "Storage health behavior" ||
                reportedCategory == "Boot and startup failure" ||
                reportedCategory == "Thermal condition"
            ))
        {
        ...
        else
        {
                ...
                !strongUserWarning)
            {
                ...
                diagnosedCategory == "System log event flags")
            {
                actionCategory =
                    "Troubleshoot";
            }
            ...
            /*
             * The CPU frequency observation is heuristic and should
             * lead to troubleshooting rather than automatically
             * declaring overheating or hardware failure.
             */
            if (
                metrics.CpuThermalThrottling &&
                diagnosedCategory ==
                    "Thermal condition")
            {
                actionCategory =
            ...
            /*
             * One high process-memory reading cannot establish
             * a confirmed memory leak.
             */
            if (
                !string.IsNullOrEmpty(
                    metrics.MemoryLeakWarning) &&
            ...
            /*
             * An abnormal Windows-reported physical-drive health
             * state is serious enough to recommend backup and
             * professional verification, but it is not presented
             * as confirmed SMART predictive failure.
             */
            if (
                metrics.SmartDriveWarning &&
                diagnosedCategory ==
                    "Storage health behavior")
            {
                actionCategory =
        ...
        var target =
            _GetVerificationTarget(
                diagnosedCategory,
                proof,
                evidenceForResult);

        var nextStep =
            _GetRecommendedNextStep(
                diagnosedCategory,
                actionCategory,
                proof,
                evidenceForResult);
    ...
    private static DiagnosticProofItem _MakeUserProof(
        string label,
        string? value,
        string meaning)
    {
    ...
    private static List<DiagnosticProofItem> _SelectDisplayProof(
        string category,
        List<DiagnosticProofItem> proof,
        List<DiagnosticEvidenceItem> evidence)
    {
        var display =
            new List<DiagnosticProofItem>();
            ...
            return proof
                .Where(
                    p =>
                        p.Label == "CPU load" ||
                        p.Label == "RAM usage" ||
                        p.Label == "Storage usage" ||
                        p.Label == "Browser workload")
                .ToList();
        }
                ...
                else
                {
                    display.Add(
                        _MakeProof(
                            ...
                            "observed",
                            healthWarning != null
                                ? "Storage capacity is not critically full, but Windows reported an abnormal physical-drive health status."
    ...
    private static string _GetRecommendedNextStep(
        string category,
        ...
        List<DiagnosticEvidenceItem> evidence)
    {
        ...
        if (cat.Contains("os performance"))
        {
            var highMemoryObservation =
                proof.Any(
                    p =>
                        p.Label ==
                            "Unusually High Process Memory Usage");

            if (highMemoryObservation)
            {
                return "Open Task Manager and monitor the high-memory application. A single reading does not confirm a memory leak, so check whether its memory usage continues to grow over time. Close or restart the application if it is causing performance problems.";
            }
        ...
        {
            return "Reduce heavy CPU workloads and check that vents and fans are unobstructed. The current scan observed possible frequency throttling but did not directly confirm overheating. Monitor the system again, and seek inspection if repeated shutdowns, severe heat, or other cooling symptoms occur.";
        }

        if (cat.Contains("driver"))
        {
        ...
        if (cat.Contains("storage"))
        {
            ...
            if (hasHealthWarning)
            {
                return "Back up important files promptly. Windows reported an abnormal physical-drive health status, but this scan does not directly confirm a SMART predictive failure. Verify the drive with a dedicated storage-health or SMART diagnostic tool before deciding whether replacement is necessary.";
            }
        ...
        if (
            actionCategory
                ...
                .Contains("escalate"))
        {
            return "Stop repeated testing and bring this result to a qualified technician, especially if the computer cannot boot, shows severe heat-related symptoms, or reports hardware warning signs.";
        }

        return "Review the proof shown above, use the safest available action, and run the diagnosis again after the change.";
    }

    private static VerificationTarget _GetVerificationTarget(
        string category,
        List<DiagnosticProofItem> proof,
        List<DiagnosticEvidenceItem> evidence)
    {
        ...
        if (cat.Contains("no active issue"))
        {
            return new VerificationTarget
            {
                ...
                Label =
                    "No verification target needed",

                Description =
                    "The live scan did not find an active issue that needs a Windows tool right now."
            };
        }
            ...
            cat.Contains("startup"))
        {
                    ...
                    "startup repair"))
            {
                return new VerificationTarget
                {
                    ...
                    Description =
                        "Use this if Windows still opens. It can show recent startup failures, crashes, or update-related problems."
                };
            }
            ...
            return new VerificationTarget
            {
                ...
                Description =
                    "Review apps that run during startup."
            };
        }
        ...
        if (cat.Contains("os performance"))
        {
                    ...
                    "Browser workload"))
            {
                return new VerificationTarget
                {
                    ...
                    Description =
                        "Check which applications are using the most memory or CPU and monitor whether unusually high usage persists."
                };
            }
            ...
            return new VerificationTarget
            {
                ...
                Description =
                    "Review apps that automatically run when Windows starts."
            };
        }
        ...
        if (cat.Contains("thermal"))
        {
            return new VerificationTarget
            {
                ...
                Description =
                    "Review CPU utilization and reported frequency. The current evidence suggests possible frequency throttling but does not directly prove overheating."
            };
        }
        ...
        if (cat.Contains("storage"))
        {
            ...
            return new VerificationTarget
            {
                ...
                Description =
                    hasHealthWarning
                        ? "Back up important data and verify the physical drive with a dedicated storage-health or SMART diagnostic tool."
                        : "Inspect drive space before running any read-only disk check."
            };
        }
            ...
            cat.Contains("driver"))
        {
            return new VerificationTarget
            {
                ...
                Description =
                    "Inspect the driver without changing it automatically."
            };
        }
        ...
        return new VerificationTarget
        {
            ...
            Description =
                "Review recent crashes and Windows error events."
        };
    }
}